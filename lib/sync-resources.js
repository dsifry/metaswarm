...
function checkTomlCommands() {
  ...
  for (const [name, def] of Object.entries(TOML_COMMAND_MAP)) {
    ...
    if (!fs.existsSync(tomlPath)) {
      ...
      if (!content.includes(def.description)) {
        console.error(`DRIFT: commands/metaswarm/${name}.toml description mismatch`);
        issues++;
      }
    }
  }
...
function validateManifests() {
  ...
  if (fs.existsSync(pkgPath)) {
    versions['package.json'] = JSON.parse(fs.readFileSync(pkgPath, 'utf-8')).version;
  }
  if (fs.existsSync(pluginPath)) {
    versions['plugin.json'] = JSON.parse(fs.readFileSync(pluginPath, 'utf-8')).version;
  }
  if (fs.existsSync(geminiPath)) {
    versions['gemini-extension.json'] = JSON.parse(fs.readFileSync(geminiPath, 'utf-8')).version;
  }
...
function check() {
  ...
  if (drifted > 0) {
    console.error(`\n${drifted} issue(s) found. Run: node lib/sync-resources.js --sync`);
    process.exit(1);
  }
