...
const { allPlatforms } = require('./platform-registry');
...
// --- Multi-format command generation ---
//
// One TOML_COMMAND_MAP feeds every CLI's command directory. Each platform's
// `commandFormat` field (in lib/platform-registry.js) selects which generator
// produces its files:
//
//   gemini-toml   → commands/metaswarm/<name>.toml  (Gemini extension format)
//   opencode-md   → .opencode/commands/<name>.md    (OpenCode markdown + YAML)
//
// To add a new format: add a generator entry to FORMAT_GENERATORS below
// and set the new platform's commandFormat to that key.
...
const FORMAT_GENERATORS = {
  ...
  'opencode-md': {
    fileExt: '.md',
    generate(name, def) {
      // Convert Gemini-style {{args}} placeholder into OpenCode's $ARGUMENTS.
      const body = def.prompt.replace(/\{\{args\}\}/g, '$ARGUMENTS');
      // Description must be a YAML scalar — escape quotes so it's safe inline.
...
// Returns the list of platforms that have a generated command format.
function commandTargets() {
  return allPlatforms()
    .filter(p => p.commandFormat && p.commandsDir)
    .map(p => ({
      key: p.key,
      dir: p.commandsDir,
      generator: FORMAT_GENERATORS[p.commandFormat],
      formatKey: p.commandFormat,
    }))
    .filter(t => {
      if (!t.generator) {
        console.error(`WARNING: ${t.key} declares commandFormat=${t.formatKey} but no generator is registered.`);
        return false;
      }
      return true;
    });
...
function syncCommandsForTarget(target) {
  const dir = path.join(ROOT, target.dir);
  fs.mkdirSync(dir, { recursive: true });
  let synced = 0;

  for (const [name, def] of Object.entries(TOML_COMMAND_MAP)) {
    const filePath = path.join(dir, `${name}${target.generator.fileExt}`);
    const content = target.generator.generate(name, def);
...
function checkCommandsForTarget(target) {
  const dir = path.join(ROOT, target.dir);
  let issues = 0;
  ...
  for (const [name, def] of Object.entries(TOML_COMMAND_MAP)) {
    ...
    if (!fs.existsSync(filePath)) {
      console.error(`MISSING: ${target.dir}/${name}${target.generator.fileExt}`);
      issues++;
    } else {
      const content = fs.readFileSync(filePath, 'utf-8');
      const expected = target.generator.generate(name, def);
      if (content !== expected) {
        console.error(`DRIFT: ${target.dir}/${name}${target.generator.fileExt} content mismatch`);
        issues++;
      }
    }
  }
...
// Sync every platform that has a commandFormat. Returns { totalSynced,
// perTarget: { <key>: count } } so callers can produce the same
// "70 file(s) (including 12 TOML, 12 OpenCode)" log line.
function syncAllCommands() {
  const perTarget = {};
  let totalSynced = 0;
  for (const target of commandTargets()) {
    perTarget[target.key] = syncCommandsForTarget(target);
    totalSynced += perTarget[target.key];
  }
  return { totalSynced, perTarget };
}

function checkAllCommands() {
  let issues = 0;
  for (const target of commandTargets()) {
    issues += checkCommandsForTarget(target);
  }
  return issues;
}
...
function check() {
  ...
  // Check generated commands across every registered platform format
  drifted += checkAllCommands();
...
function sync() {
  ...
  // Sync generated commands across every registered platform format
  const { totalSynced, perTarget } = syncAllCommands();
  synced += totalSynced;

  const breakdown = Object.entries(perTarget)
    .map(([key, count]) => `${count} ${key}`)
    .join(', ');
  console.log(`Synced ${synced} file(s) (including ${breakdown}).`);
}
