...
function parseYaml(text) {
  ...
  return parseBlock(entries, 0, entries.length, -1);
}
 ...
 */
function parseBlock(entries, start, end, parentIndent) {
  const result = {};
  let i = start;
  ...
  while (i < end) {
    ...
    if (content.startsWith('- ')) {
      // Shouldn't hit arrays at object level — skip
      i++;
      continue;
    }
    ...
    if (rest !== '') {
      ...
      if (childStart < childEnd) {
        ...
        if (entries[childStart].content.startsWith('- ')) {
          ...
          result[key] = parseBlock(entries, childStart, childEnd, indent);
        }
      } else {
        result[key] = null;
      }
...
function parseArray(entries, start, end, arrayIndent) {
  ...
  while (i < end) {
    ...
    if (content.startsWith('- ')) {
      ...
      // Check if the array item has nested content (i.e., it's "- key: value" or just "- value")
      if (itemContent.includes(':') && !itemContent.startsWith('"') && !itemContent.startsWith("'")) {
        // Could be an object item in array — for our use case, just treat as scalar
